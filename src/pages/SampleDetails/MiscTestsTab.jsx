  ...
  return (
    <div className="space-y-6">
      ...
      <Section title="Antibacterial Assay">
        {antibacterialRuns.length === 0 ? <Empty /> : (
          <div className="space-y-3">
            {antibacterialRuns.map((run, index) => (
              <div key={run.id || index} className="bg-white border border-gray-100 rounded-xl p-5">
                ...
                <TestIdBadge run={run} label="ABSY ID" />
                <div className="space-y-3">
      ...
      <Section title="Antimalarial Assay">
        {antimalarialRuns.length === 0 ? <Empty /> : (
          <div className="space-y-3">
            {antimalarialRuns.map((run, index) => (
              <div key={run.id || index} className="bg-white border border-gray-100 rounded-xl p-5">
                ...
                <TestIdBadge run={run} label="AASY ID" />
                <div className="space-y-3">
      ...
      <Section title="Biochemical Tests">
        {biochemicalRuns.length === 0 ? <Empty /> : (
          <div className="space-y-3">
            {biochemicalRuns.map((run, index) => (
              <RunDisplay key={run.id || index} run={run} index={index} color="green" label="BT ID" />
            ))}
          </div>
        )}
      </Section>
      ...
      <Section title="Enzymatic Tests">
        {enzymaticRuns.length === 0 ? <Empty /> : (
          <div className="space-y-3">
            {enzymaticRuns.map((run, index) => (
              <RunDisplay key={run.id || index} run={run} index={index} color="purple" label="EBT ID" />
            ))}
          </div>
        )}
      </Section>
    ...
    <div className="bg-purple-50 border border-purple-100 rounded-lg px-4 py-3 mb-4">
      <div className="flex flex-wrap items-start gap-6">
        ...
        <div>
          ...
          <p className="font-mono font-semibold text-purple-700 text-sm break-all">{run.testId}</p>
        </div>
      </div>
    </div>
  );
...
/* ================= RUN DISPLAY ================= */
function RunDisplay({ run, index, color, label }) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-5">
      ...
      <TestIdBadge run={run} label={label} />
      {allTests.length === 0 ? (
