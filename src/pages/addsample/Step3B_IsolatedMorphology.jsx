...
import { generateIsoMorId, getNextIsoMorIndex } from "../../utils/sampleIdGenerator";
...
export default function Step3B_IsolatedMorphology() {
  ...
  /* ================= ADD RUN ================= */
  const addRun = () => {
    const nextIndex = getNextIsoMorIndex(runs);
    const firstIsoId = isolatedOptions[0]?.value || "";
    const isoMorId = firstIsoId
      ? generateIsoMorId(firstIsoId, nextIndex)
      : "";
  ...
  const updateIsoLink = (id, isolatedId) => {
    ...
    const isoMorId = isolatedId
      ? generateIsoMorId(isolatedId, index + 1)
      : "";
    updateRuns(
      ...
      {isolatedOptions.length === 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3">
          <p className="text-yellow-700 text-sm">
            No primary isolated entries found — go to Step 3A first to add isolated entries before adding morphology.
          </p>
        </div>
      )}
                  ...
                  <CollapsibleSection title="Macroscopic Morphology">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      <SelectInput label="Shape"
                        value={run.macroscopic?.shape || ""}
                        options={MACROSCOPIC_SHAPES}
                        onChange={(v) => updateField(run.id, "macroscopic", "shape", v)} />
                      <SelectInput label="Arrangement"
                        value={run.macroscopic?.arrangement || ""}
